...
Beta_OrientData <- read.csv('Beta_OrientData.csv')

```

############################ ROIs plot ####################################
Plot BetaWeight of stimulus types by ROIs
```{r Beta_Distribution}

Beta_OrientData$reROIs <- revalue(Beta_OrientData$ROIs, c("arFFA"="rFFA", "prFFA"="rOFA"))

meanBetaOrient <- droplevels(ddply(Beta_OrientData, .(reROIs, Subj, orient), summarise,
                                   N = length(unique(voxel)),
                                   avgBeta=mean(Beta),
                                   avgBetaSte=mean(BetaSte)))
                                    

avgSubjmeanBeta <- ddply(meanBetaOrient, .(reROIs, orient), summarise,
                       N=length(Subj),
                       std=sd(avgBeta),
                       se=std/sqrt(N),
                       avgAllSubjBeta = mean(avgBeta))


# plot single subj Beta
dev.new()
ggplot(meanBetaOrient, aes(x = orient, y = avgBeta)) + 
  theme_few() + 
  facet_wrap(Subj ~reROIs, scales='free') + 
  #geom_errorbar(aes(ymin=avgBeta-avgBetaSte, ymax=avgBeta+avgBetaSte), width=0.1) +
  geom_bar(stat='identity')


# plot avg of all subjects
dev.new()
ggplot(avgSubjmeanBeta, aes(x=orient, y=avgAllBeta)) + 
  theme_few() + 
  facet_wrap(~reROIs, scales='free') + 
  geom_smooth(aes(group=1), method='loess', se=FALSE) + 
  geom_errorbar(aes(ymin=avgAllBeta-se, ymax=avgAllBeta+se), width=0.1) +
  #geom_point(size=5)
  geom_bar(stat='identity')
